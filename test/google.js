...
(async () => {
  ...
  const searchInput = guestPage.getByRole('combobox', { name: 'Tìm kiếm' });

  await guestMouse.type(searchInput, "67", 50);
  ...
  // 1. Get all buttons that match the role
  const allButtons = guestPage.getByRole('button', { name: 'Tìm trên Google' });

  // 2. Find the one that is currently visible
  let visibleButton;
  ...
  if (!visibleButton) {
    throw new Error("Could not find a visible Google search button!");
  }

  // 3. Now perform the mouse action
  await guestMouse.clickHumanRandom(visibleButton);
