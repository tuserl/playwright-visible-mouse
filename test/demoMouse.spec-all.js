...
test.describe("DemoMouse", () => {
  ...
  test("moveTo() moves to element center", async ({ page }) => {
    await mouse.moveTo("#btn");
  ...
  test("click() clicks target element", async ({ page }) => {
    let clicked = false;
  ...
  test("type() enters text", async ({ page }) => {
    await mouse.type("#input", "Hello");

    await expect(page.locator("#input")).toHaveValue("Hello");
  });

  test("moveTo() throws if element doesn't exist", async () => {
    await expect(mouse.moveTo("#missing")).rejects.toThrow(
      "Element not found"
    );
  });
});
