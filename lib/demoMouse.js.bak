...
class DemoMouse {
  ...
  async moveToPosition(x, y) {
    // 1. Update the Node.js memory tracker so it doesn't snap on reload
    this.lastX = x;
    this.lastY = y;

    // 2. Move the visual anime sprite in the browser
    await this.page.evaluate(({ x, y }) => {
      if (window.moveFakeMouse) window.moveFakeMouse(x, y);
    }, { x, y });

    // 3. Move Playwright's actual backend mouse matching your turbo speed
