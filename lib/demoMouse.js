...
class DemoMouse {
  ...
  async install() {
    const injectMouse = async () => {
      try {
        ...
        const css = getSpriteVariablesCss(config);
        ...
        await this.page.addStyleTag({
          path: path.join(
            __dirname,
            config?.isArrow ? "demoMouse-arrow.css" : "demoMouse.css"
          )
        });

        const hasCustomSprites =
          config?.idleImage || config?.clickImage;
        ...
        await this.page.evaluate(({ lastX, lastY }) => {
          ...
          // Global Escape key listener to toggle mouse visibility
          window.addEventListener("keydown", (e) => {
            if (e.key === "Escape") {
              // Check if the hide-cursor class is currently present
              const isHidden = document.body.classList.contains('hide-cursor');

              if (isHidden) {
                document.body.classList.remove('hide-cursor'); // Make it appear
                console.log("Mouse visible");
              } else {
                document.body.classList.add('hide-cursor');    // Make it disappear
        ...
        console.error("[DemoMouse ERROR]", err);
      }
    };
    await injectMouse();
  ...
  async show() {
    await this.page.evaluate(() => document.body.classList.remove('hide-cursor'));
  }

  async hide() {
    await this.page.evaluate(() => document.body.classList.add('hide-cursor'));
  }
  ...
  async setLockState(isLocked, notFollowUser = false) {
    await this.page.evaluate(({ isLocked, notFollowUser }) => {
      ...
      if (mouseEl) {
        //mouseEl.style.pointerEvents = !isLocked ? "auto" : "none";// (!isLocked && notFollowUser) ? "auto" : "none";
